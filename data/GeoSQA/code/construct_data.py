...
def parse(row):
    ...
    pattern = r"Image Annotation\s*:?\s*(.*?)\s*Scenario\s*:?\s*(.*?)\s*Question\s*:?\s*(.*?)\s*Answers?\s*:?\s*(.*)"
    match = re.search(pattern, text, re.DOTALL)
    if match:
        image_annotation, scenario, question, answers = match.groups()
        row['annotation'] = image_annotation.strip()
...
if __name__ == '__main__':
    ...
    print(data["translated_text"][954])
